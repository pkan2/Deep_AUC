...
def _fully_connected(input_layer, num_labels):
    with tf.name_scope('fully_connected'):
     fc_h = tf.compat.v1.layers.dense(input_layer, num_labels)
    return fc_h
...
def _batch_norm(input_layer, phase_train):
    bn_layer = tf.compat.v1.layers.batch_normalization(inputs=input_layer, training=phase_train, fused=None, momentum=_batch_norm_decay, epsilon=_batch_norm_epsilon)
    return bn_layer

def _avg_pool( x, pool_size, stride):
    with tf.name_scope('avg_pool'):
      x = tf.compat.v1.layers.average_pooling2d(x, pool_size, stride, 'SAME')
    return x
...
def _residual_v1(x,
    ...
    with tf.name_scope('residual_v1'):
      ...
      with tf.compat.v1.variable_scope('sub_1'):
          x = _conv(x, kernel_size, out_filter, stride)
          x = _batch_norm(x, phase_train)
          x = _elu(x)
      with tf.compat.v1.variable_scope('sub_2'):
...
def resnet_inference(input_tensor_batch, num_layers, num_classes=10, activations='elu', phase_train=True):
    ...
    with tf.compat.v1.variable_scope('resblock_%d'%(0)):
        x = _conv(x, 3, 16, 1)
        x = _batch_norm(x, phase_train=phase_train)
        x = _elu(x)
        
       
    with tf.compat.v1.variable_scope('res_block_%d'%(1)):
        for j in range(n):
            with tf.compat.v1.variable_scope('layer_%d'%(j)):
                if j == 0:
    ...
    with tf.compat.v1.variable_scope('res_block_%d'%(2)):
        for j in range(n):
            with tf.compat.v1.variable_scope('layer_%d'%(j)):
                if j == 0:
                    # First block in a stage, filters and strides may change.
    ...
    with tf.compat.v1.variable_scope('res_block_%d'%(3)):
        for j in range(n):
            with tf.compat.v1.variable_scope('layer_%d'%(j)):
                if j == 0:
                    # First block in a stage, filters and strides may change.
